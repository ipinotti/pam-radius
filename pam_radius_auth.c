...
get_random_vector(unsigned char *vector)
{
  ...
  DPRINT(LOG_DEBUG, "get_random_vector: ");
  for (i = 0; i < AUTH_VECTOR_LEN; i+=4)
	  DPRINT(LOG_DEBUG, "%02x %02x %02x %02x ", vector[i], vector[i+1], vector[i+2], vector[i+3]);
}
...
add_password(AUTH_HDR *request, unsigned char type, CONST char *password, char *secret)
{
  ...
  if (!attr) {
	  int i;
	  DPRINT(LOG_DEBUG, "add_password:  ");
	  for (i = 0; i < length; i+=4)
		  DPRINT(LOG_DEBUG, "%02x %02x %02x %02x ", hashed[i], hashed[i+1], hashed[i+2], hashed[i+3]);
...
build_radius_packet(AUTH_HDR *request, CONST char *user, CONST char *password, radius_conf_t *conf)
{
   ...
   */
  if (password) {
	  DPRINT(LOG_DEBUG, "building radius packet : pass %s secret %s\n", password, conf->server->secret);
...
static int rad_converse(pam_handle_t *pamh, int msg_style, char *message, char **password)
{
  ...
  if (password) {		/* assume msg.type needs a response */
    ...
    *password = resp->resp;
    DPRINT(LOG_DEBUG, "Got password from conversation : %s\n", *password );
    free(resp);
  }
